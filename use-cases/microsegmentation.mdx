...
Anyone who has a need to segment containerized systems in their organization can use [Calico's network policies](/calico/latest/network-policy/get-started/calico-policy/calico-network-policy) to isolate, protect and secure Kubernetes workloads, namespaces, tenants, and hosts.
...
For example, consider a frontend and a backend microservice in the same namespace; they are in separate security domains and must be protected by network policies.
...
To implement microsegmentation, you should follow a structured and repeatable approach to increase likelihood of success.
...
As policies are identity-aware, it is likely assets within a cluster will be targeted using labels.
...
In Calico Cloud and Calico Enterprise the [staging policy](/calico-enterprise/latest/reference/resources/stagednetworkpolicy) tool will help you find incorrect and missing policy; a global deny helps mitigate against other lateral malicious attacks.
...
In Calico Cloud and Calico Enterprise, the policies board will be the first sign of misconfigurations, making it easy to spot policies that are denying traffic or that have no endpoints assigned to them.

#### Use observability tools

Calico Enterprise and Calico Cloud have multiple features that allow easy visual identification of potentially misconfigured policies: Service Graph, FlowViz, Policy Board, Dashboards.
...
#### Test connectificy with curl or netcat

While testing policies, additional pods can deployed within different security domains to use curl or netcat to test connectivity to different pods or services.
...
Using and testing the deployed application and a wide range of its functionality should show whether there are any issues after deploying policies.
